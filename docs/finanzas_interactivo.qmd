...
Estas funciones centralizan la descarga de precios y el cálculo de retornos.  
La idea es que **todo lo demás** (estimadores, simulación, LS, binomial) dependa de aquí, para no repetir lógica.

---

## get_close {#func-get-close}

**Objetivo:** descargar precios de Yahoo Finance y devolver un `DataFrame` con una columna por ticker.

**Recibe:**
- `tickers` (`str` o `list[str]`): símbolo(s) (ej. `"AAPL"` o `["AAPL","SBUX"]`)
- `fecha_inicio` (`str`): fecha inicio, formato `"YYYY-MM-DD"`
- `fecha_fin` (`str`): fecha fin, formato `"YYYY-MM-DD"`
- `auto_adjust` (`bool`, default `True`):
  - Si `True`, yfinance ajusta el precio por splits/dividendos (según la lógica de Yahoo/yfinance).
  - Si `False`, te deja precios sin ajuste

**Que hace:**
1. Normaliza `tickers` a lista.
2. Llama a `_download_close_cached(...)`, que:
   - Descarga datos con `yf.download(...)`.
   - Maneja los dos formatos de salida típicos:
     - **MultiIndex** (varios tickers): columnas tipo `("Close", "AAPL")`, etc.
     - **SingleIndex** (un ticker): columnas simples tipo `"Close"`.
   - Extrae la serie de `Close` (o `Adj Close` si fuera lo disponible).
   - Asegura que el resultado final sea un `DataFrame` con columnas = tickers.
   - Limpia filas totalmente vacías (`dropna(how="all")`).
3. Valida que todas las columnas pedidas estén presentes:
   - Si falta algún ticker, lanza error con columnas disponibles.
4. Devuelve el `DataFrame` únicamente con las columnas solicitadas (en el orden solicitado).

**Devuelve:**
- `pd.DataFrame` indexado por fecha (trading days), con columnas exactamente `tickers`.
  - Para un ticker: `DataFrame` de una columna (no `Series`).

**Nota:**
`_download_close_cached` usa `@lru_cache`, así que si vuelves a pedir **los mismos tickers y mismas fechas**, no vuelve a descargar: reutiliza el resultado en memoria (más rápido y reproducible dentro de la misma sesión).
...
**Objetivo:** calcular retornos a partir de un DataFrame de precios close.

**Recibe:** 
- `close: DataFrame` :  precios con fechas en el índice y activos en columnas.
- `kind (str, default "log")`: 
     "log": retornos logaritmicos $r_t =  log(S_t) - log(S_{t-1})$ 
     Otro valor:  retornos simples $ r_t =  \frac{S_t}{S_{t-1}} - 1 $
**Hace:**

1.- Elimina filas con `NaN` en cualquiera de las columnas (dropna(how="any")): Esto fuerza a que todos los activos queden alineados en las mismas fechas.
...
3.-  Elimina el primer renglon(queda `NaN` por la diferencia) y cualquier otro `NaN` residual. 

**Devuelve:** `pd.DataFrame` con retornos(mismo formato de columnas que `close`), ya limpio y alineado.
